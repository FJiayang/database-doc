...
public class GenerateService {
    ...
    public void generate() throws IllegalAccessException {
        loadDatafile();
        // 遍历所有数据库
        properties.getDb().forEach((k, v) -> {
            HikariConfig hikariConfig = new HikariConfig();
            hikariConfig.setDriverClassName("com.mysql.cj.jdbc.Driver");
            hikariConfig.setJdbcUrl(v.getAddress() + k);
            hikariConfig.setUsername(v.getUsername());
            hikariConfig.setPassword(v.getPassword());
            //设置可以获取tables remarks信息
            hikariConfig.addDataSourceProperty("useInformationSchema", "true");
            hikariConfig.setMinimumIdle(2);
            hikariConfig.setMaximumPoolSize(5);
            // 表集合
            Collection<String> tableList = tableMap.get(k);
            screw(new HikariDataSource(hikariConfig), tableList, k);
        });
    }
    ...
    private void screw(DataSource dataSource, Collection<String> tableList, String databaseName) {
        ...
        // 支持通配符
        if (tableList.size() == 1 && tableList.contains("*")) {
